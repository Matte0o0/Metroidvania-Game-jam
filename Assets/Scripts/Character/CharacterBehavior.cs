...
//defines general behavior of NPC and player. Will decide later if I want to add additional script for each personalised behavior
public class CharacterBehavior : MonoBehaviour
{

    public Rigidbody2D rb;
    public SkeletonAnimation skeletonAnimation;
    private string currentAnimation = "idle"; // Default idle animation
    ...
    [SerializeField] public float smoothFactor = 10f;
    ...
    public void MoveCharacter(float moveInput)
    {
        ...
        {
            currentSpeed = 0;  // Instantly stop when changing direction
        }

        if (moveInput != 0)
        {
            currentSpeed = Mathf.Lerp(currentSpeed, moveInput * speed, acceleration * Time.deltaTime);
        }
        else
        {
    ...
    //not really needed, should change
    public void HandleMovementAnimation(float moveInput)
    {
        if (Mathf.Abs(moveInput) > 0.01f) // check if character is moving, if it is, set the walk animation
        {
            ...
            SetCharacterAnimation("walk", true);
            Flip(moveInput);
        }
        else
        {
    ...
    public void Idle()
    {
        ...
        {
            SetCharacterAnimation("idle", true);
        }
    }

    //may need a similar one for the roof
    ...
    void OnDrawGizmos()
    {
        ...
        // Set the color based on the grounded state
        Gizmos.color = isGrounded ? Color.green : Color.red;

        // Draw a sphere at the ground check position to visualize the grounded state
        Gizmos.DrawWireSphere(groundCheckPoint.position, 0.1f);

        // You can also visualize the rays by drawing lines
        Gizmos.color = isGrounded ? Color.green : Color.red;
    ...
    public void Hit(Transform _attackTransfom, Vector2 _attackArea)
    {
        ...
        if (objectsToHit.Length > 0)
        {
            ...
            {
                if (objectsToHit[i].GetComponent<EnemyBehavior>() != null)
                {
                    Debug.Log("hit landed");
                    //objectsToHit[i].GetComponent<EnemyBehavior>().GetHit(damage);
                    objectsToHit[i].GetComponent<EnemyController>().TakeDamage(damage);
                }
            }
        }
    ...
    //careful, this triggers for any NPC death, along with the player one
    public void Death()
    {
        //if (health <= 0)
        //{
